...
namespace Nodes_satisfy_private
...
end Nodes_satisfy_private

def Graph.contains (g  : Graph) (exp : List Int) : Bool :=
  if let some b := g.asBool
  then b
  else Nodes_satisfy_private.satisfy_graph g (Ref.last g.nodes) exp
